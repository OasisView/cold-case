...
// Every function: try/catch, typed result, TODO comment marks where to swap mock → real
...
// ── Supabase client (exported for direct use in server components if needed) ──
// Table name matches loader.py HOMICIDES_TABLE = 'homicides'
...
export async function getClusters(
  ...
  // TODO: swap for real Supabase call when Manny's loader confirms Green River smoke test passes
  // Real query: supabase.from('homicides').select(...).eq('state', filters.state)...
  try {
